...
def upload(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            resultFullPath = getFilePath(resultFilename)
...
def run_scheduler(filename, in_nSessions, in_workshopNames, useMetadata):
    dataLines = []
    uploadReader = csv.reader(open(getFilePath(filename), 'r'), delimiter=',', quotechar='|')
    for line in uploadReader:
        dataLines.append(line)

    resultFilename = initialiseAndRunScheduler(filename, in_nSessions, in_workshopNames, useMetadata)
    resultLines = []
    resultReader = csv.reader(open(getFilePath(resultFilename), 'r'), delimiter=',', quotechar='|')
...
def handle_uploaded_file(f):
    ...
    destination = open(getFilePath(tempLocation), 'wb+')
